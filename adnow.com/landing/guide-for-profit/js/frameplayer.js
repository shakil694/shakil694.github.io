...
FramePlayer.prototype.poster = function(obj, w, h) {
    var img = new Image();
    img.src = obj;
    img.onload = function(){
    ...
    if (_HTTP) {
        ...
        if (typeof(_HTTP.onload) !== undefined) {
            _HTTP.onload = function() {
                ...
                var iPh = $('.iphone'),
                    firstClass = iPh.attr('class');

                setTimeout(function(){
                    iPh.attr('class', 'iphone');
                    $('.video-options').find('.option').removeClass('show active');
                    iPh.removeClass('hide');
                    $('.iphone-event-layer').removeClass('active');
                    $('#my-player').find('canvas').remove();
                    setTimeout(function(){
                      iPh.find('.video-wrapper').show();
                    }, 500);
                }, 15000);
            };
        } else {
